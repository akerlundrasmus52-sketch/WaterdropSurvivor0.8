...
if (window.BloodSystem) {
  // Use BloodV2 direct hit for realistic wound tracking
  if (window.BloodV2 && typeof window.BloodV2.hit === 'function') {
    var _wk = weapons && weapons.gun && weapons.gun.level >= 5 ? 'revolver' : 'pistol';
    // Build a minimal enemy proxy for BloodV2
    var _bv2enemy = {
      alive: !slot.dead,
      enemyType: 'slime',
      id: 'slime_' + slot._poolIdx,
      hp: slot.hp,
      maxHp: slot.maxHp,
      mesh: slot.mesh,
      velocity: { x: slot.knockbackVx || 0, y: 0, z: slot.knockbackVz || 0 }
    };
    var _bv2hit = { x: _bloodPos.x, y: _bloodPos.y, z: _bloodPos.z };
    var _bv2normal = { x: projectile ? -projectile.vx : 0, y: 0, z: projectile ? -projectile.vz : 0 };
    window.BloodV2.hit(_bv2enemy, _wk, _bv2hit, _bv2normal);
  } else if (typeof BloodSystem.emitBurst === 'function') {
...
if (window.BloodSystem) {
  // Use BloodV2.kill for realistic death explosion
  if (window.BloodV2 && typeof window.BloodV2.kill === 'function') {
    var _bv2killEnemy = {
      alive: false,
      enemyType: 'slime',
      id: 'slime_kill_' + slot._poolIdx + '_' + Date.now(),
      hp: 0, maxHp: slot.maxHp,
      mesh: slot.mesh,
      velocity: { x: killVX || 0, y: 0, z: killVZ || 0 }
    };
    var _wkKill = weapons && weapons.gun && weapons.gun.level >= 5 ? 'revolver' : 'pistol';
    window.BloodV2.kill(_bv2killEnemy, _wkKill);
  }
